...
import {
  ...
  Plus, Award, CheckSquare, Layers, X
} from "lucide-react";
...
export default function ProjectDetails() {
  ...
  async function toggleTask(milestoneId: number, task: Task) {
    ...
    // Optimistic UI update — flip it immediately
    setTasks((prev) => ({
      ...prev,
      [milestoneId]: prev[milestoneId].map((t) =>
          t.id === task.id ? { ...t, status: newStatus, completedAt: newCompletedAt } : t
      ),
    }));

    try {
      if (projectService.updateTask) {
        // Backend derives completedAt from status — send status, not completedAt directly
        await projectService.updateTask(task.id, { status: newStatus });
      }
    } catch {
      // Revert to the original values if the API request fails
      setTasks((prev) => ({
        ...prev,
        [milestoneId]: prev[milestoneId].map((t) =>
            t.id === task.id ? { ...t, status: previousStatus, completedAt: previousCompletedAt } : t
        ),
      }));
    }
  }
